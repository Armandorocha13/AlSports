...
export function transformStrapiCategory(strapiCategory: StrapiCategory): Category {
  ...
  const subcategories = (subcategoriesData?.data || []).map((sub: any) => {
    ...
    // Buscar imagem de capa - pode vir como 'image', 'ImagemDaSubcategoria' ou 'Imagem'
    const subcategoryImage = subAttrs.ImagemDaSubcategoria || subAttrs.image || subAttrs.Imagem
    return {
      id: sub.documentId || sub.id?.toString() || '',
      name: subAttrs.name || subAttrs.Nome || '',
      slug: subAttrs.slug || '',
      image: getImageUrl(subcategoryImage),
    }
  })
