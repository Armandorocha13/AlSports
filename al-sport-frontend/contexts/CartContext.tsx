...
export function CartProvider({ children }: CartProviderProps) {
  ...
  const createOrder = async (orderData: any): Promise<{ success: boolean; error?: string }> => {
    ...
    try {
      ...
      console.log('📝 Dados do pedido a serem inseridos:', JSON.stringify(orderInsert, null, 2))
      ...
      if (orderError) {
        ...
        console.error('❌ Dados tentados:', JSON.stringify(orderInsert, null, 2))
        
        // Verificar se é erro de RLS
        if (orderError.code === '42501' || orderError.message?.includes('permission denied') || orderError.message?.includes('RLS')) {
          console.error('⚠️ ERRO DE PERMISSÃO (RLS): O usuário pode não ter permissão para inserir pedidos.')
          console.error('⚠️ Verifique as políticas RLS da tabela "orders" no Supabase.')
        }
