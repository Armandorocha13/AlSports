...
export default function FavoritesPage() {
  ...
  const { favorites, removeFromFavorites, clearFavorites, refreshFavorites, isLoading } = useFavorites()
  const { addItem } = useCart()
  
  // Recarregar favoritos do banco quando a página for montada
  useEffect(() => {
    console.log('FavoritesPage - Montada, recarregando favoritos do banco...')
    refreshFavorites()
  }, [refreshFavorites]) // Recarregar quando a função mudar (mas só na montagem)
  
  // Debug: log dos favoritos sempre (não só em desenvolvimento)
  useEffect(() => {
    console.log('FavoritesPage - Favoritos recebidos:', favorites.length)
    if (favorites.length > 0) {
      console.log('FavoritesPage - IDs dos favoritos:', favorites.map(f => f.id))
      console.log('FavoritesPage - Nomes dos favoritos:', favorites.map(f => f.name))
    } else if (!isLoading) {
      console.log('FavoritesPage - Nenhum favorito encontrado')
    }
  }, [favorites, isLoading])
